function SankeyUtil(opts) {
  this.el = opts.el;
  this.width = opts.layoutStyle.width || 600;
  this.height = opts.layoutStyle.height || 400;
  this.margin = opts.layoutStyle.margin || {top: 1, right: 1, bottom: 1, left: 1};
  this.dataNodes = opts.data.nodes || [];
  this.dataLinks = opts.data.links || [];
  this.nodeCallback = opts.nodeCallback || function () {};
}
SankeyUtil.prototype.render = function () {
  var self = this
  // set color formate
  var formatNumber = d3.format(",.0f"),   //decimal places
  format = function(d) { return formatNumber(d) + ""; },
  color = d3.scaleOrdinal(d3.schemeCategory20);
  d3.select("#"+self.el).append('div').attr("class", "tooltip")
  //  set svg container
  ...
  var toolTips = d3.select("#"+self.el).select(".tooltip");
  sankey.nodes(self.dataNodes)
...
SankeyUtil.prototype.update = function (data) {

};



var sankeyUtil = function () {
  var control = {};
  control.render = function (opts) {
